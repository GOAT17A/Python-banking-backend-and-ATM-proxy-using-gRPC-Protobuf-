...
```bash
./.venv/bin/python main.py bank-server --listen 127.0.0.1:50051 --db bank.db
```

### 2) Start the ATM server (proxy)

```bash
./.venv/bin/python main.py atm-server --listen 127.0.0.1:50052 --bank 127.0.0.1:50051
```

### 3) Create a customer + account (Bank Admin)

```bash
./.venv/bin/python main.py bank-admin --bank 127.0.0.1:50051
```
...
```bash
./.venv/bin/python main.py atm --atm 127.0.0.1:50052
```
###.python -m pytest -q
